...
% edges(1,1) = max(min(data_X/mean(data_X)),0.4);
% edges(1,1) = min(data_X/mean(data_X)); % fixed lower boundary = min(data)
...
%     edges(1,p) = edges(1,1) + (p-1)*(max(data_X/mean(data_X))-edges(1,1))/(N_edges-1); % fixed upper boundary = max(data)
...
%     if isempty(bin_X{1,p}) == 0 && l(p)>50 %fixed number (for Sattar's data)
        binned_data(1,p) = mean(bin_X{1,p}(:,1));
        binned_data(2,p) = std(bin_X{1,p}(:,1));
        
        binned_data(3,p) = mean(bin_X{1,p}(:,2));
        binned_data(4,p) = std(bin_X{1,p}(:,2));
