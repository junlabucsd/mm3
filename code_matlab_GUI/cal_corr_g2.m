function [pool_data, pool_data_norm, pool_CV, pool_corr, L_corr] = cal_corr_g2(input_strct)
...
pool_data = ones(length(input_strct.division_length_g2),9);
pool_data_norm = pool_data;
pool_CV = zeros(1,12);
pool_data(:,1) = input_strct.septum_position_g2;
pool_data(:,2) = input_strct.elongation_rate_fit_g2;
pool_data(:,3) = input_strct.initiation_mass_g2;
pool_data(:,4) = input_strct.tau_cyc_g2;
pool_data(:,5) = input_strct.B_period_g2;
pool_data(:,6) = input_strct.C_period_g2;
pool_data(:,7) = input_strct.D_period_g2;
pool_data(:,8) = input_strct.division_length_g2;
pool_data(:,9) = input_strct.newborn_length_g2;
pool_data(:,10) = input_strct.generation_time_g2;
pool_data(:,11) = pool_data(:,8) - pool_data(:,9);
pool_data(:,12) = 2*input_strct.initiation_mass_g3 - input_strct.initiation_mass_g2;
% % pool_data(:,12) = log(pool_data(:,8)./pool_data(:,3));
% pool_data(:,12) = 2*input_strct.division_length_g3 - input_strct.newborn_length_g2;
% pool_data(:,12) = pool_data(:,8) - pool_data(:,3);
% pool_data(:,12) = double(input_strct.initiation_time_g3 - input_strct.initiation_time_g2); %initiation frequency
% pool_data(:,12) = pool_data(:,2).*pool_data(:,4);
