function [length_list, foci_list, birth_list, division_list, cell_list, cell_names, save_name, save_name_png, display_name] = plot_channel(dir_name, cell_data, px_to_mu, IW_thr, fnames_sort, channels, channle_idx)
...
for k = 1:L_channles
    fname_rec = ['f' num2str(fnames_channel(k,1),'%.2d') 'p' num2str(fnames_channel(k,2),'%.4d') 't' num2str(fnames_channel(k,3),'%.4d') 'r' num2str(fnames_channel(k,4),'%.1d')];
    cell_temp = cell_data.(fname_rec);
        ...
        if fnames_channel(k,4) ==1 && length(cell_temp.times)>=2 && isempty(cell_temp.disp_l)==0 %only look at mother cells and only those have two daughter cells and are with fluorescence
                    ...
                    if isempty(cell_temp.disp_l{1,p})==0
                        for q=1:length(cell_temp.disp_l{1,p})
                            if cell_temp.foci_h4{1,p}(1,q)>=IW_thr
                                ...
                                h3.Color = [1 0 1]; set(h3,'LineWidth',1,'Markersize',2,'Marker','o','MarkerFaceColor',[1 1 1],'LineStyle','None');
                                ...
                                h4.Color = [1 0 1]; set(h4,'LineWidth',1,'Markersize',2,'Marker','o','MarkerFaceColor',[1 1 1],'LineStyle','None');
            ...
            ylim([-1 5])
            set(gca,'YScale','linear','YTick',[0 1 2 3 4],'YTickLabel',{'0','1','2','3','4','5'},'YGrid','Off');
