...
if __name__ == "__main__":
    ...
    # switches
    try:
        opts, args = getopt.getopt(sys.argv[1:],"f:sxu")
    except getopt.GetoptError:
        print('no arguments detected (-f).')
    for opt, arg in opts:
        if opt == '-f':
            param_file = arg # parameter file path
        if opt == '-s':
    ...
    # m = Manager()
    # q = m.Queue()
    pool = Pool(num_analyzers) # pool for mulitprocessing (finding metadata, then saving images)
    ...
    try:
        ...
        if len(found_files) > 0:
            ...
            for fn in found_files:
                ...
                analyzed_imgs[fn] = pool.apply_async(get_params, [fn, True])

            pool.close() # tells the process nothing more will be added.
            pool.join() # blocks script until everything has been processed.
